...
Block capabilities use verified Data block/Text block types and the explicit Collection data source relation. Metadata queries omit Collection item edges; those are fetched through the separate cursor reader. Unsupported blocks retain Geo links. Missing linked blocks and datasets retain unavailable states rather than disappearing; a successful membership removal still removes them. Browser response-interception checks verified unsupported-source changes, unavailable dataset selection and recovery, without Geo writes. Build, formatting and 62 tests pass.
