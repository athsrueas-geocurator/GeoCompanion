...
Version 0.2.0 adds About and Recent changes within Preferences. An accessible dot marks unseen feature releases for returning browsers; opening Recent changes clears it. The separate local read marker, actual deployment dates and release procedure are documented in [release history](docs/RELEASES.md).
...
Saved names are resolved only when Preferences or Curation renders the followed profiles, not at root or outreach startup. Identical successful query/variable combinations are cached in memory for five minutes, capped at 50 entries. Reload clears the response cache but preserves selected IDs. Public profile data remains canonical on Geo. Search changes abort pending fetches and suppress stale responses; requests time out after 15 seconds. Profile following adds no polling, push notifications or service worker. GraphQL errors/HTTP errors are distinct from empty results; a missing/deleted profile keeps its stored ID with an unavailable state.
