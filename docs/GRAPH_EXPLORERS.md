...
## Fullscreen and entity tables — 0.4.7

Root cause: requestFullscreen was called on a dialog, an explicitly unsupported element under the Fullscreen API. The previous browser-dependent diagnosis was incomplete. A normal div inside the modal now owns fullscreen; fullscreenchange updates the Exit fullscreen control, and close exits the owned fullscreen context. Verified true fullscreen entry in the Chromium in-app browser, not merely viewport expansion. Reference: https://developer.mozilla.org/en-US/docs/Web/API/Element/requestFullscreen.

The force view now shows an accessible scrollable property table on hover, tap or keyboard selection. It stays readable while moving into the panel; a click/keyboard selection latches the node until dismissal. Background click and the panel close button clear selection. A 300ms delay cancels transient hover reads; stale responses cannot replace the current selection. The shared two-minute memory cache deduplicates reads and does not cache failures. No persistent entity detail storage is introduced.

The exact-ID scoped query reads 32 values and 20 outgoing relationships, retaining their published names and repeated rows, numeric zero, false, dates/times and units. Known contact/wallet/address fields and email-like text are omitted. Unsupported binary, embedding, point and schedule payloads are not rendered. Truncation links readers to Geo rather than claiming complete coverage. HTML is rendered as text. No inference about types, relationships or identity is made. Live Value introspection verified date, datetime, time, float and unit. STAR test read returned source-table text, factual classification, description and supporting/opposing/source relationships.
