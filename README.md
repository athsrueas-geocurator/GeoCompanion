...
Current implementation: [graph explorers](docs/GRAPH_EXPLORERS.md) and [larger graphs with opt-in caching](docs/GRAPH_CACHE_DESIGN.md). React Flow is the default; force-graph is an optional lazy-loaded pop-out.
...
Source now includes shared map controls for education and outreach. See [Deployment](DEPLOYMENT.md) for the latest verified hosted version and [map layer plan](docs/MAP_LAYERS_PLAN.md) for implemented capabilities and remaining phases. The endpoint currently used is Geo **testnet**.
